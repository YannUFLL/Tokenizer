...
* **BONUS - MultiSig Decentralized Custody:** To eliminate the "Single Point of Failure" inherent in standard wallets, administrative control (contract onlyOwner) has been transferred to a custom **Multi-Signature (MultiSig) Vault**.
...
* **Hardhat Configuration Variables:** (`vars`)** To avoid the security risks of `.env` files (such as accidental leaks to GitHub), I managed sensitive data like private keys and owner addresses using Hardhat’s built-in encrypted variable storage.
